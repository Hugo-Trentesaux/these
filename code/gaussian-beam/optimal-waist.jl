...
a = map(x -> gaussian_width(d,x,λ1), waists)
b = map(x -> gaussian_width(d,x,λ2), waists)
...
# savefig("gaussianbeamoptimalwaist.png")
...
# wa = λ1 ./ (pi .* θ)
# wb = λ2 ./ (pi .* θ)
# a = map(x -> gaussian_width(d,x,λ1), wa)
# b = map(x -> gaussian_width(d,x,λ2), wb)
# # θ waists*1e6
# # xlabel!("N.A")