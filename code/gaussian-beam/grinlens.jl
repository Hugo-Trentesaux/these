...
function Parameters(;
    ...
    return Parameters(λ,P,k,α,dndT,focale,z0,w0,l,steps)
end
...
Observables(n::Int) = Observables(zeros(n),zeros(n),zeros(n),zeros(n),zeros(n),zeros(n),zeros(n)) # init arrays


# simulate a gaussian beam propagation in water
function propagate(λ,focale,z0,w0,l,steps)
    # observables
...
propagate(p::Parameters) = propagate(p.λ, p.focale, p.z0, p.w0, p.l, p.steps)